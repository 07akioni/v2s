...
exports.transformScript = function transformScript (
  ...
  traverse(ast, {
    ImportDeclaration (path) {
      ...
      // rewrite .vue exts
      path.node.source.value = refactorSource(include, dir, path.node.source.value)
    },
    CallExpression(path) {
      if (path.node.callee.type === 'Import') {
        // @ts-ignore
        path.node.arguments[0].value = refactorSource(include, dir, path.node.arguments[0].value)
      }
    },
